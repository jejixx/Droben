...
/**
 * Calcule la position cible du rôle custom (juste au-dessus du Premium, sous le bot).
 * @param {import('discord.js').Role} premiumRole
 * @param {import('discord.js').Role} botHighestRole
 * @returns {number|null}
 */
function computeCustomRolePosition(premiumRole, botHighestRole) {
  let target = premiumRole.position + 1;
  const botPosition = botHighestRole.position;

  if (target >= botPosition) {
    target = botPosition - 1;
  }

  if (target <= premiumRole.position) {
    return null;
  }
...
/**
 * Place le rôle custom au-dessus du Premium pour que sa couleur soit visible.
 * @param {import('discord.js').Guild} guild
 * @param {import('discord.js').Role} customRole
 * @param {import('discord.js').Role|null|undefined} premiumRole
 */
async function positionCustomRoleAbovePremium(guild, customRole, premiumRole) {
  if (!premiumRole) {
    logWarn(
      `Rôle Premium introuvable (${config.PREMIUM_ROLE_ID}) — rôle custom créé sans repositionnement.`,
    );
    return;
  }
  ...
  if (!botMember) {
    logWarn('Membre bot introuvable — repositionnement du rôle custom ignoré.');
    return;
  }

  const target = computeCustomRolePosition(premiumRole, botMember.roles.highest);

  if (target == null) {
    logWarn(
      `Impossible de placer « ${customRole.name} » au-dessus du Premium (hiérarchie insuffisante sous le rôle du bot).`,
    );
    return;
  }

  try {
    await customRole.setPosition(target, {
      reason: 'SUPP GAP Premium — couleur visible au-dessus du rôle Premium',
    });
  } catch (error) {
    logError('perks:positionCustomRole', error);
  }
}
 ...
 * @param {import('discord.js').Role|null|undefined} premiumRole
 */
async function ensureCustomRole(guild, member, existingRoleId, roleName, roleColor, premiumRole) {
  if (existingRoleId) {
  ...
  await positionCustomRoleAbovePremium(guild, role, premiumRole);
...
async function provision(client, payload) {
  ...
  const premiumRole = await guild.roles.fetch(config.PREMIUM_ROLE_ID).catch(() => null);

  if (!premiumRole) {
    logWarn(`Rôle Premium introuvable (${config.PREMIUM_ROLE_ID}) — attribution Premium ignorée.`);
  } else if (!member.roles.cache.has(premiumRole.id)) {
