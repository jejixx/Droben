...
/**
 * Place le rôle custom juste sous le rôle du bot pour que sa couleur l'emporte
 * sur les rôles membres (Premium, booster, etc.) tout en restant sous le staff.
 * @param {import('discord.js').Guild} guild
 * @param {import('discord.js').Role} customRole
 */
async function positionCustomRoleBelowBot(guild, customRole) {
  ...
  if (target < 1) {
    logWarn(
      `Impossible de placer « ${customRole.name} » sous le rôle du bot (position invalide : ${target}).`,
    );
    return;
  }
  ...
  try {
    await customRole.setPosition(target, {
      reason: 'SUPP GAP Premium — couleur visible sous le rôle du bot',
    });
  } catch (error) {
...
async function ensureCustomRole(guild, member, existingRoleId, roleName, roleColor) {
  ...
  await positionCustomRoleBelowBot(guild, role);
